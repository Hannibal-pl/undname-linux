...
//		flags |= UNDNAME_NO_FUNCTION_RETURNS | UNDNAME_NO_ACCESS_SPECIFIERS | UNDNAME_NO_MEMBER_TYPE | UNDNAME_NO_ALLOCATION_LANGUAGE | UNDNAME_NO_COMPLEX_TYPE;
//	}

	sym.flags         = UNDNAME_COMPLETE;
	sym.current       = argv[1];
	str_array_init( &sym.names );
	str_array_init( &sym.stack );

	printf("%s\n",symbol_demangle(&sym) ? sym.result : argv[1]);
	return 0;
}
